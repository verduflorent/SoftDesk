...
class SoftDeskUserJourneyTests(APITestCase):
    def setUp(self):
        ...
        Contributor.objects.create(user=self.owner, project=self.project)
        Contributor.objects.create(user=self.member, project=self.project)
    ...
    def test_project_creation_adds_creator_as_contributor(self):
        ...
        self.assertTrue(Contributor.objects.filter(user=self.outsider, project=project).exists())
